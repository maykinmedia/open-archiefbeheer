...
class ExternalRegistersConfigurationStepTests(ClearCacheMixin, TestCase):
    def test_configure_external_registers(self):
        ServiceFactory.create(slug="openproduct_service1")
        ServiceFactory.create(slug="openproduct_service2")
        ...
        self.assertEqual(
            ...
            ["openproduct_service1", "openproduct_service2"],
        )
    ...
    def test_missing_services(self):
        with self.assertRaises(
            ConfigurationRunFailed,
            msg="Missing services with slugs: openproduct_service1, openproduct_service2."
