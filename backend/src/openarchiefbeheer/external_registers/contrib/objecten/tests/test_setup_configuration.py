...
CONFIG_FILE_PATH = str(TEST_FILES / "data.yaml")
BROKEN_CONFIG_FILE_PATH = str(TEST_FILES / "broken_data.yaml")


class ExternalRegistersConfigurationStepTests(ClearCacheMixin, TestCase):
    def test_configure_external_registers(self):
        ServiceFactory.create(slug="objecten_service1")
        ServiceFactory.create(slug="objecten_service2")
        ...
        self.assertEqual(
            ...
            ["objecten_service1", "objecten_service2"],
        )
    ...
    def test_missing_services(self):
        with self.assertRaises(
            ConfigurationRunFailed,
            msg="Missing services with slugs: objecten_service1, objecten_service2."
