...
``src/openarchiefbeheer/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test openarchiefbeheer
...
* ``DB_NAME``: name of the database for the project. Defaults to ``openarchiefbeheer``.
* ``DB_USER``: username to connect to the database with. Defaults to ``openarchiefbeheer``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``openarchiefbeheer``.
...
1. Clone or download the code from `Github`_ in a folder like
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/openarchiefbeheer.git
       Cloning into 'openarchiefbeheer'...
       ...
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting openarchiefbeheer_db_1 ... done
       Starting openarchiefbeheer_web_1 ... done
   ...
   .. code-block:: bash
       ...
       $ docker logs -f openarchiefbeheer_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it openarchiefbeheer_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it openarchiefbeheer_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/openarchiefbeheer/
...
environment variables. See ``src/openarchiefbeheer/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t openarchiefbeheer
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    $ docker exec -it openarchiefbeheer /app/src/manage.py createsuperuser
