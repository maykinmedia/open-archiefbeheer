...
The fixtures in `./fixtures` are NOT automatically loaded when the Open Zaak container starts.
These are used by the tests to reload data specific to a particular test. See the developer documentation on
VCR tests for more details.

Some older fixtures were generated by running Open Zaak locally and using the factories to create the 
various objects. The newer ones were generated with the `OpenZaakDataCreationHelper`. 

The fixture ``complex_data.json`` contains the following objects and relations:
